const BASE_URL =
  "https://pluckyinnovation.com/crankyoldmangaming/CrankyMarvel/CrankyComics/";
...
export function parseRow($, row) {
  ...
  const { runDate, sampleCount, confidence, prices } = parsePricesCell(
    cells.eq(5).text(),
  );
...
function parsePricesCell(cellText) {
  ...
  const runDate = dateStr.includes("/")
    ? new Date(
        `20${dateStr.split("/")[2]}-${dateStr.split("/")[0]}-${dateStr.split("/")[1]}`,
      ).toISOString()
    : null;

  const priceRegex = /([\d.]+):\$([\d.]+)[\s ]*\/[\s ]*\$([\d.]+)/g;
  const prices = [...cellText.matchAll(priceRegex)].map(
    ([, grade, raw, slab]) => ({
